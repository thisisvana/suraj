...
<?php
	...
	if(ISSET($_POST['name'])){
		...
		$name = $_POST['name'];
		$link = $_POST['link'];
		$connection->query("INSERT INTO simulations (`name`, `video`, `description`) VALUES('$name', '$link', '$post')") or die(mysqli_error());
	}
?>
<?php
	if(ISSET($_POST['res'])){
		$q_post = $connection->query("SELECT * FROM simulations ORDER BY `id` DESC") or die(mysqli_error($connection));
